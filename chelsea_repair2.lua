-- chelsea_repair2.lua -- release raw-added players, then native cTransferPlayer re-sign + status setters
...
-- inlined v2 helper: write squad role into PlayerStatusManager (type 89)
...
function main()
    log("== CHELSEA REPAIR2 START ==")
    for _, ent in ipairs(TRANSFERS) do
        local pid, tid = ent.pid, ent.tid
        log(ent.name .. " pid=" .. pid .. " -> " .. tid .. " wage=" .. ent.wage .. " months=" .. ent.months)
        ...
        -- release frees the squad slot the raw write occupied
        local okr = pcall(ReleasePlayerFromTeam, pid)
        log("ReleasePlayerFromTeam -> " .. tostring(okr))

        -- native sign-on: creates the real contract + status manager entries
        local ok = pcall(cTransferPlayer, pid, FREE, tid, 0, -1, ent.wage, ent.months)
        log("cTransferPlayer -> " .. tostring(ok))

        -- manager writes: role Rotation, morale/sharpness/fitness
        local okr2 = SetSquadRole(pid, 3)
        local okm = pcall(SetPlayerMorale, pid, 75)
        local oks = pcall(SetPlayerSharpness, pid, 60)
        local okf = pcall(SetPlayerFitness, pid, 85)
        log("status -> role=" .. tostring(okr2) .. " morale=" .. tostring(okm) .. " sharp=" .. tostring(oks) .. " fit=" .. tostring(okf))
        log(ent.name .. " DONE")
    end
    log("== CHELSEA REPAIR2 DONE ==")
end
