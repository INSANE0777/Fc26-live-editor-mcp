...
text = text.replace(old_norm_team, new_norm_team)

old_search = '''function handlers.search_players(cmd)
...
new_search = '''function handlers.search_players(cmd)
    local name = cmd.name or ""
    if name == "" then return { success = false, error = "name required" } end
    ensure_cache_loaded()
    local limit = cmd.limit or 20
...
old_list = '''function handlers.list_team_players(cmd)
    ...
    return { success = true, teamid = teamid, teamname = teamname, players = results, count = #results }
end'''
new_list = '''function handlers.list_team_players(cmd)
    local teamid = cmd.teamid or normalize_team_arg(cmd.team)
    if not teamid then return { success = false, error = "team or teamid required" } end
    ensure_cache_loaded()
    local ok, rows = pcall(GetDBTableRows, "teamplayerlinks")
    ...
    local team_entry = TEAM_CACHE[teamid]
    local teamname = team_entry and team_entry.teamname or safe_name(GetTeamName(teamid))
    local results = {}
    for _, row in ipairs(rows) do
        if tonumber(row.teamid.value) == teamid then
            local pid = tonumber(row.playerid.value)
            local entry = PLAYER_CACHE[pid]
            table.insert(results, {
                playerid = pid,
                name = entry and entry.name or safe_name(GetPlayerName(pid)),
                teamid = teamid,
                teamname = teamname
            })
        end
...
old_ping = '''function handlers.ping(cmd)
    return { success = true, message = "Live Editor bridge is running" }
end'''
new_ping = '''function handlers.ping(cmd)
    return { success = true, message = "Live Editor bridge is running" }
end
...
pp = Path("C:/fc26-mcp/src/fc26_mcp/mcp_le_bridge.py")
...
ptext = ptext.replace(old_tools, new_tools)
